...
if (isset($_GET['blogId'])) {
    $sql = "SELECT blogpost.*,user.username AS creator_name FROM blogpost JOIN user ON  user.id=blogpost.creator WHERE blogpost.creator=$userId AND blogpost.blog_id=$blogId";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($blogTitle)  && isset($description) && isset($content) && isset($category)) {
        ...
        WHERE blog_id = $blogId AND creator = $userId";
...
<body>
    ...
    <div class="form-container">
        ...
        <form id="form" action="editBlog.php?blogId=<?php echo $blogId; ?>" method="POST" enctype="multipart/form-data">
            <p class="err"><?= $generalErr; ?></p>
            <div class="form-group">
                <label for="title">Blog Title</label>
                <input value="<?= $blogPost['title'] ?>" type="text" id="title" name="title" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <img src="<?= $blogPost['blogImage'] ?>" id="avatar">
            </div>
            <div class="form-group">
                <label for="description">Description</label>
                <textarea id="description" name="description" rows="10" required><?= $blogPost['description'] ?></textarea>
            </div>
            <div class="form-group">
